...
git clone https://github.com/ahargunyllib/banking-peak-load-prototype.git
...
The remote runner scripts also trap `Ctrl+C`/SSH disconnects and use a watchdog to stop their child `k6` process, so an interrupted Ansible run should not leave load running in the background.
